...
'''
if in_notebook_mode:
    model_name = "pythia-160m"#"pythia-160m"####
    BATCH_SIZE = 5
...
# %% Helper Functions
'''
def new_logits_upon_ablation_calc(model, clean_tokens, corrupted_tokens, ABLATION_TYPE, logits: Tensor, heads = None, mlp_layers = None, num_runs = 1, clean_cache: Union[None, ActivationCache] = None):
    """
...
def get_thresholded_change_in_logits(model, clean_tokens, corrupted_tokens, ABLATION_TYPE, logits: Tensor, per_component_direct_effect: Tensor, heads = None, mlp_layers = None, thresholds = [0.5], cache: Union[None, ActivationCache] = None):
    """"
    ...
    change_in_logits = new_logits_upon_ablation_calc(model, clean_tokens, corrupted_tokens, ABLATION_TYPE, logits, heads, mlp_layers, clean_cache=cache) - get_correct_logit_score(logits, clean_tokens)
...
def generate_results(model, ABLATION_TYPE):
    PROMPT_LEN = 128
    BATCH_SIZE = 30
    ...
    for batch_idx, clean_tokens, corrupted_tokens in dataset:
        ...
        for layer in range(model.cfg.n_layers):
            for head in range(model.cfg.n_heads):
                dict_results = get_thresholded_change_in_logits(model, clean_tokens, corrupted_tokens, ABLATION_TYPE, logits, per_head_direct_effect, heads = [(layer, head)], thresholds = THRESHOLDS, cache = cache)
    ...
    return thresholded_de, thresholded_cil, thresholded_count


# %% Plotting functionality
def gpt_new_plot_thresholded_de_vs_cre(thresholded_de, thresholded_cre, thresholds, model_name = "pythia-160m", use_logits = False, layout_horizontal = True):
    """
...
def write_result(model, ABLATION_TYPE, model_name = 'pythia-160m', FOLDER_TO_WRITE_GRAPHS_TO = "figures/new_self_repair_graphs/", FOLDER_TO_STORE_PICKLES= "pickle_storage/new_graph_pickle/" ):
    ...
    THRESHOLDS = [0.0 * i for i in range(0,12)]           
    ...
    with open(FOLDER_TO_STORE_PICKLES + type_modifier + f"{model_name}_thresholded_de_{thresholds_str}.pkl", "wb") as f:
        pickle.dump(thresholded_de, f)
    # Serialize and save thresholded_ci
    with open(FOLDER_TO_STORE_PICKLES + type_modifier + f"{model_name}_thresholded_cil_{thresholds_str}.pkl", "wb") as f:
